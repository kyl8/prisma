...
import { AppNotification, Product } from "./data";
  ...
  return (
    <>
      <div className="ws-page-title">
        ...
        <button
          className="ws-quiet"
          onClick={() => markAllNotificationsRead(companyId)}
        >
...
function ImpAssistant({
  ...
  const [conversations, setConversations] = useState([
    "Pendências do meu catálogo",
    "Produtos para corrigir",
    "Como enviar para revisão",
    "Status dos aprovados",
  ]);
  const { products } = useStoreState();
  const mine = products.filter((p) => p.companyId === companyId);
  const needs = mine.filter((p) => p.status === "needs_you");
...
export function ImporterPortal({ onLogout }: { onLogout: () => void }) {
  ...
  const { notifications, products } = useStoreState();
  const go = (v: ImpView) => {
    setView(v);
    setNotice(false);
  ...
  const Page = () => {
    switch (view.name) {
      ...
      case "pending":
        return <ImpPending go={go} companyId={company.id} />;
      case "notifications":
        return (
          <ImpNotifications
            go={go}
        ...
        {chat && (
          ...
          >
            ...
            <p>
              ...
              <b>
                {view.name === "product"
                  ? (myProducts.find((p) => p.id === view.productId)?.name ??
                    company.name)
                  : company.name}
              </b>
            </p>
            ...
            <div className="ws-chat-input">
              <input placeholder="Pergunte sobre seu catálogo…" />
