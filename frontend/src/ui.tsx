...
export function NoticePopover({
  ...
  return (
    <div className="ws-popover notice">
      <h3>Notificações</h3>
      {error && <div role="alert"><p>{error}</p>{onRetry && <button type="button" className="ws-quiet" onClick={onRetry}>Tentar novamente</button>}</div>}
...
export function Dropdown({
  ...
  const triggerText = label
    ? !showSelectedValue || hideAllLabel && isAll
      ? label
