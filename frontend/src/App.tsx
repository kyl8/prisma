...
import { signInWithCredentials, signOutFromCredentials } from "./api/importer";
import {
  login,
  logout,
  useCurrentUser,
      ...
      >
        <form
          ...
          onSubmit={(event) => {
            ...
            setLoading(true);
            setError("");
            onEnter(email, password).catch((cause: Error) => setError(cause.message)).finally(() => setLoading(false));
          }}
        >
          <p className="eyebrow">ACESSO À PLATAFORMA</p>
          <h1>Entre no seu workspace.</h1>
          <p>
            Entre com o e-mail e a senha cadastrados na plataforma.
          </p>
          <label>
            E-mail
            <input
          ...
          <button className="button button--dark" type="submit" disabled={loading}>
            {loading ? "Entrando…" : "Entrar na plataforma"} {!loading && <Icon name="arrow" size={17} />}
          </button>
...
export function App() {
  ...
  const handleLogin = async (email: string, password: string) => {
    const authenticated = await signInWithCredentials(email, password);
    login(authenticated.role === "importer" ? "u-mariana" : "u-carlos");
  };
  const handleLogout = async () => {
    await signOutFromCredentials().catch(() => undefined);
    logout();
  };
  useEffect(() => {
  ...
  if (route.name === "app") {
    return (
      <>
        {user ? (
          user.role === "importer" ? (
            <ImporterPortal onLogout={handleLogout} />
          ) : (
        ...
        <section className="final-cta" data-scroll-section>
          <Reveal>
            ...
            <a
              ...
              href="mailto:p.henriqueteixeira001@gmail.com"
            >
              Conversar com a PRISMA <Icon name="arrow" size={18} />
            </a>
            <p>PortoHack Santos 2026</p>
          </Reveal>
        </section>
      </main>
      <ToastHost />
