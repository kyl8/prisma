...
export function CatalogRequestFlow({ token }: { token: string }) {
  ...
  const saveDraftProducts = async (draft: Record<string, Record<string, string>>) => {
    let latestRequest = request;
    for (const [productId, attributes] of Object.entries(draft)) {
      latestRequest = await saveCatalogProduct(token, productId, attributes);
    }
    return latestRequest;
  };
  return <RequestShell><motion.div className="req-card req-card--catalog" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}><CatalogBatchForm products={products} sending={sending} onSave={async (draft) => { setSending(true); try { setRequest(await saveDraftProducts(draft)); showToast("Progresso salvo. Você pode continuar pelo mesmo link."); playUISound("save"); } catch (e) { showToast(e instanceof Error ? e.message : "Não foi possível salvar."); playUISound("error"); } finally { setSending(false); } }} onSubmit={async (draft) => { setSending(true); try { await saveDraftProducts(draft); const submitted = await submitCatalogRequest(token); setRequest(submitted); playUISound("submit"); } catch (e) { showToast(e instanceof Error ? e.message : "Preencha os campos obrigatórios."); playUISound("warning"); } finally { setSending(false); } }} /></motion.div></RequestShell>;
}
