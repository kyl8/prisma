...
export function presentActivity(activity: ActivityDTO, viewerId?: string) {
  ...
  switch (activity.type) {
    ...
    case "CATALOG_INCONSISTENCY_FOUND": return { title: metadata.source === "SISCOMEX" ? "Comparação SISCOMEX requer revisão" : `Sistema encontrou ${inconsistencyCount ?? ""} inconsistências`.replace("  ", " "), subtitle: product ?? fileName ?? activity.company.name };
  }
}
